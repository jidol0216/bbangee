...
class FaceDetectionNode(Node):
    ...
    def __init__(self):
        ...
        # 2단계 분리 판별 + 시간적 안정화 설정
        self.iff_band_threshold = 0.65   # Stage 1 밴드 유무 임계값 (65%)
        self.iff_text_threshold = 0.60   # Stage 2 텍스트 분류 임계값 (60%)
        self.iff_history = []            # 최근 IFF 결과 히스토리
        self.iff_history_size = 10       # 히스토리 크기 (약 0.3초)
        self.iff_consensus_ratio = 0.7   # 다수결 비율 (70%)
    ...
    def classify_iff(self, frame, detection):
        ...
        try:
            # 2단계 분리 판별로 CNN 분류
            iff, band, conf, stage_info = self.iff_classifier.classify_iff(
                frame, 
                band_threshold=self.iff_band_threshold,
                text_threshold=self.iff_text_threshold
            )
            
            # 디버그: 각 단계 상세 결과
            s1_no = stage_info.get('stage1_no_band', 0)
            s1_band = stage_info.get('stage1_has_band', 0)
            s2_tongil = stage_info.get('stage2_tongil', 0)
            ...
            # 히스토리에 추가 (시간적 안정화)
            self.iff_history.append({'iff': iff, 'band': band, 'conf': conf})
            if len(self.iff_history) > self.iff_history_size:
                self.iff_history.pop(0)
            
            # 다수결로 안정화된 결과 계산
            stable_result = self._get_stable_iff_result()
            
            if stable_result:
                self.last_iff_result = stable_result
                return stable_result
            else:
                # 아직 안정화 안됨 - 현재 결과 반환하되 UNKNOWN 표시
                result = {'iff': 'UNKNOWN', 'band': band, 'conf': conf, 'stable': False}
    ...
    def _get_stable_iff_result(self):
        """시간적 안정화: 최근 N프레임 중 다수결로 결과 확정"""
        if len(self.iff_history) < self.iff_history_size // 2:
            return None  # 데이터 부족
        
        # IFF 결과별 카운트
        from collections import Counter
        iff_counts = Counter(r['iff'] for r in self.iff_history)
        band_counts = Counter(r['band'] for r in self.iff_history)
        
        # 가장 많은 결과
        most_common_iff, iff_count = iff_counts.most_common(1)[0]
        most_common_band, band_count = band_counts.most_common(1)[0]
        
        # 다수결 비율 충족 체크
        total = len(self.iff_history)
        iff_ratio = iff_count / total
        band_ratio = band_count / total
        
        if iff_ratio >= self.iff_consensus_ratio:
            # 평균 confidence 계산
            matching = [r for r in self.iff_history if r['iff'] == most_common_iff]
            avg_conf = sum(r['conf'] for r in matching) / len(matching)
            
            return {
                'iff': most_common_iff,
                'band': most_common_band,
                'conf': avg_conf,
                'stable': True,
                'ratio': iff_ratio
            }
        ...
        if detection:
            ...
            if self.iff_classifier is not None:
                ...
                if iff_result:
                    ...
                    cv2.putText(frame, f"Band: {band_str}",
